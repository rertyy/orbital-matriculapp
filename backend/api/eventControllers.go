...
// TODO check if the http code I am sending is correct

func (h *Handler) HandleGetAllEvents(w http.ResponseWriter, r *http.Request) {
	log.Println("getAllEvents")

	sqlStatement := `SELECT e.event_id, e.event_name, e.event_body, e.event_start_date, e.event_end_date
					FROM events e
...
-- 					WHERE ukt.user_id = 1;` // TODO change this to the user id
	...
	for rows.Next() {
		var event Event
		if err := rows.Scan(&event.EventId, &event.Name, &event.Body, &event.StartDate, &event.EndDate); err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
